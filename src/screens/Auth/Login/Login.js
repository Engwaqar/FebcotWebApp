...
const Login = ({ navigation }) => {
  ...
  const getCompanies = async () => {
    try {
      const res = await Api.get(urls.GET_DEPARTMENT);
  ...
  const Validation = (item) => {
    navigation.navigate('DraweerTabs');
            ...
            <View style={styles.footer}>
              ...
              <View style={{ marginTop: hp(2), marginHorizontal: wp(5) }}>
                ...
                <Input
                  ...
                  tintColor={colors.black}
                  leftIcon={globalPath.password}
                />
                <View style={{ marginTop: hp(2)}}>
                  <DropDown
                  data={data.map((v)=>v.name)}
                  defaultButtonText={"Select Department"}
                  leftIcon={globalPath.arrowdropdown}
                  onSelect={(item) => {
                    var id = data.find(
                      v => v.name == item,
                  )?.id;
